...
class CorrBlock1:
    def __init__(self, fmap1, fmap2, region1, region2, region3, region4,num_levels=4, radius=4,dataset='kitti'):
        ...
        if self.dataset=='kitti':
            corr = CorrBlock1.corr1_k(fmap1, fmap2, region1, region2, region3, region4)
        else:
            corr = CorrBlock1.corr1_s(fmap1, fmap2, region1, region2, region3, region4)
    ...
    @staticmethod
    def corr1_k(fmap1, fmap2, region1, region2, region3, region4):
    ...
    @staticmethod
    def corr1_s(fmap1, fmap2, region1, region2, region3, region4):
        batch, dim, ht, wd = fmap1.shape
        fmap1 = fmap1.view(batch, dim, ht * wd).to(device)

        region1[:, :, 0:ht // 4 + 8, 0:wd] = fmap2[:, :, 0:ht // 4 + 8, 0:wd]
        region1[:, :, ht // 4 + 8:ht // 2, 0:wd] = 0.75 * fmap2[:, :, ht // 4 + 8:ht // 2, 0:wd]  #
        region1[:, :, ht // 2:ht // 2 + 8, 0:wd] = 0.5 * fmap2[:, :, ht // 2:ht // 2 + 8, 0:wd]

        region2[:, :, ht // 4 - 8:ht // 2 + 8, 0:wd] = fmap2[:, :, ht // 4 - 8:ht // 2 + 8, 0:wd]
        region3[:, :, ht // 2 - 8:3 * ht // 4 + 8, 0:wd] = fmap2[:, :, ht // 2 - 8:3 * ht // 4 + 8, 0:wd]

        region4[:, :, ht // 4:ht // 2 + 8, 0:wd] = 0.75 * fmap2[:, :, ht // 4:ht // 2 + 8, 0:wd]
        region4[:, :, ht // 2 + 8:ht, 0:wd] = fmap2[:, :, ht // 2 + 8:ht, 0:wd]
